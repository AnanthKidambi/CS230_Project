			...
			{ // read miss
				...
				if (miss_handled)
				{
					...
					if (RQ.entry[index].type == LOAD)
					{
						...
						if (cache_type == IS_L1D)
							REMOVE_OLD_ADDITION(l1d_prefetcher_operate(RQ.entry[index].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type);)
							NEW_ADDITION(l1d_prefetcher_operate(RQ.entry[index].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type, RQ.entry[index].virtual_addr);)
						if (cache_type == IS_L2C)
	...
	for (uint32_t i = 0; i < MAX_READ; i++)
	{
		...
		if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0))
		{
			...
			{ // prefetch hit
				...
				{
					if (cache_type == IS_L1D)
						REMOVE_OLD_ADDITION(l1d_prefetcher_operate(PQ.entry[index].full_addr, PQ.entry[index].ip, 1, PREFETCH);)
						NEW_ADDITION(l1d_prefetcher_operate(PQ.entry[index].full_addr, PQ.entry[index].ip, 1, PREFETCH, PQ.entry[index].virtual_addr);)
					else if (cache_type == IS_L2C)
					...
					if (lower_level)
					{
						...
						else
						{
							...
							else
							{
								...
								{
									if (cache_type == IS_L1D)
										REMOVE_OLD_ADDITION(l1d_prefetcher_operate(PQ.entry[index].full_addr, PQ.entry[index].ip, 0, PREFETCH);)
										NEW_ADDITION(l1d_prefetcher_operate(PQ.entry[index].full_addr, PQ.entry[index].ip, 0, PREFETCH, PQ.entry[index].virtual_addr);)
									if (cache_type == IS_L2C)
...
int CACHE::prefetch_line(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int pf_fill_level, uint32_t prefetch_metadata, uint64_t virtual_addr)
{
	pf_requested++;
	...
	if (PQ.occupancy < PQ.SIZE)
	{
		...
		{   // change this to allow prefetch across pages
			...
			NEW_ADDITION(pf_packet.virtual_addr = virtual_addr;)
