...
#define NUM_AMAP_SET 64
#define NUM_AMAP_WAY 4
#define NUM_ENTRIES_AMAP_BLOCK 16
#define LOG2_NUM_ENTRIES_AMAP_BLOCK 4
#define LOG2_NUM_AMAP_SET 6
#define NUM_TRAIN_ENTRIES 2048

#define MAX_STREAM_SIZE 256

struct PSAMAP_Entry
{
    uint64_t counter, str_addr, index_in_stream;
    uint8_t valid;
...
struct PSAMAP_BLOCK
{
    ...
    PSAMAP_Entry *block_entries;

    PSAMAP_BLOCK()
    {
        tag = 0;
        valid = 0;
        block_entries = new PSAMAP_Entry[NUM_ENTRIES_AMAP_BLOCK];
    }
};
...
struct SPAMAP_BLOCK
{
    ...
    SPAMAP_Entry *block_entries;

    SPAMAP_BLOCK()
    {
        tag = 0;
        valid = 0;
        block_entries = new SPAMAP_Entry[NUM_ENTRIES_AMAP_BLOCK];
    }
};
...
struct PSAMAP_CACHE
{
    ...
    PSAMAP_CACHE()
    {
        block = new PSAMAP_BLOCK *[NUM_AMAP_SET];
        for (uint32_t i = 0; i < NUM_AMAP_SET; i++)
        {
            block[i] = new PSAMAP_BLOCK[NUM_AMAP_WAY];
        }
    }
};
...
struct SPAMAP_CACHE
{
    ...
    SPAMAP_CACHE()
    {
        block = new SPAMAP_BLOCK *[NUM_AMAP_SET];
        for (uint32_t i = 0; i < NUM_AMAP_SET; i++)
        {
            block[i] = new SPAMAP_BLOCK[NUM_AMAP_WAY];
        }
    }
};
// End of new addition
...
public:
    ...
    // NEW ADDITION
    PSAMAP_CACHE ps_cache;
    SPAMAP_CACHE sp_cache;
    // Training_Unit_Entry training_unit[NUM_TRAIN_ENTRIES];

    unordered_map<uint64_t, Training_Unit_Entry> training_unit;

    // different implementation of the caches
    unordered_map<uint64_t, PSAMAP_Entry> ps_cache_;
    unordered_map<uint64_t, SPAMAP_Entry> sp_cache_;

    uint64_t str_addr_counter = 0;
};
